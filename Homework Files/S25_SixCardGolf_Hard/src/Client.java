...
public class Client extends JFrame implements Runnable {
    ...
    public Client(String host) {
        ...
        board = new Square[5][3]; // create board
        ...
        for (int row = 0; row < board.length; row++) {
            ...
            Card randomlyDrawnCard = newDeck[random];
            if (row != 2)
            {
                board[row][column] = new Square(randomlyDrawnCard.toString(), row * 3 + column);
            ...
            else
            {
                if (column == 0)
                    {
                    board[row][column] = new Square("DECK", row * 3 + column);
                    boardPanel.add(board[row][column]); // add square
                    }
                else if (column == 1)
                    {
                    board[row][column] = new Square("DISC.", row * 3 + column);
                    boardPanel.add(board[row][column]); // add square
                    }
                else
                    {
    ...
    private class Square extends JPanel {
        ...
        public Dimension getPreferredSize() {
            return new Dimension(60, 60); // return preferred size
        }
        ...
        public void paintComponent(Graphics g) {
            ...
            g.drawRect(0, 0, 59, 59); // draw square
            g.drawString(mark, 22, 40); // draw mark
        }
    }
    ////////////////////////
    private Card[] createDeck()
    {
        ...
        for (CardSuite cSuite : CardSuite.values())
        {
            for (int i = 1; i <= 14; i++)
            { 
                if (i == 14) //King
                {
                    Card tempCard = new Card(i,'A',cSuite);
                    fullDeck.add(tempCard);
                }
                
                else if (i == 13) //Queen
                {
                    Card tempCard = new Card(i,'K',cSuite);
                    fullDeck.add(tempCard);
                }
                
                else if (i == 12) //Jack
                {
                    Card tempCard = new Card(i,'Q',cSuite);
                    fullDeck.add(tempCard);
                }
                
                else if (i == 11) //Ace
                {
                    Card tempCard = new Card(i,'J',cSuite);
                    fullDeck.add(tempCard);
                }
                
                else
                {
                    Card tempCard = new Card(i,'Z',cSuite);
                    fullDeck.add(tempCard);
                }
        ...
        Card[] arrayOfCards = fullDeck.toArray(new Card[fullDeck.size()]);
        return arrayOfCards;
        ///////////////////////////////////////////////////////////////////////////////////////
    }
}
